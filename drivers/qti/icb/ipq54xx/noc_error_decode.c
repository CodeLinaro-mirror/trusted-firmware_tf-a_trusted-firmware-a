...
const char * const noc_target[5][50] = {
	...
	{   /* ANOC */
	 "qhs_pcie_1lane_0_xpu_cfg",
	 "qhs_pcie_1lane_1_xpu_cfg",
	 "qhs_pcie_2lane_0_xpu_cfg",
	 "qhs_pcie_1lane_1_xpu_cfg",
	 "qns_aggr_gemnoc",
	 "srvc_aggrnoc",
	 "RESERVED",
...
{
	uint32_t ndump_tmo = HWIO_INF(GEMNOC_GEMNOC_LLCC_POC_DBG_PARAMS_HIGH, NDUMPTMO);
	uint32_t next_dump_tmo_status = 0, i = 0, ext_id = 0;
	uint64_t addr_of_violation = 0;
	uint32_t stream_low[MAX_NDUMPTMO], stream_high[MAX_NDUMPTMO];
	...
	ERROR("DumpBrief_Low = 0x%08x\n",
	      HWIO_IN(GEMNOC_GEMNOC_LLCC_POC_DBG_DUMPBRIEF_LOW));

	while (i < ndump_tmo) {
		next_dump_tmo_status =
			HWIO_IN(GEMNOC_GEMNOC_LLCC_POC_DBG_DUMPTMOSTATUS_LOW);
		while (next_dump_tmo_status == 0) {
			next_dump_tmo_status =
				HWIO_IN(GEMNOC_GEMNOC_LLCC_POC_DBG_DUMPTMOSTATUS_LOW);
		}
		ERROR("Turn: %u, NextDumpTmoStatus is %u\n", i, next_dump_tmo_status);
		stream_low[i] = HWIO_IN(GEMNOC_GEMNOC_LLCC_POC_DBG_DUMPTMOSTREAM_LOW);
		stream_high[i] = HWIO_IN(GEMNOC_GEMNOC_LLCC_POC_DBG_DUMPTMOSTREAM_HIGH);
		ERROR("DumpTmoStream%u_Low = 0x%08x\n", i, stream_low[i]);
		ERROR("DumpTmoStream%u_High = 0x%08x\n", i, stream_high[i]);
		i++;
	}
